...
const invoiceAuditLog = require("./invoiceAuditLog-_cdIy4q-.js");
require("fs");
require("path");
require("zod");
...
const createInvoiceAuditLog = async ({
  invoiceId,
  action,
  details,
  performedBy,
  ...
  return invoiceAuditLog.default.create(
    {
...
const getInvoiceAuditLogs = async (invoiceId) => {
  const where = {};
  if (invoiceId !== void 0) {
    where.invoiceId = invoiceId;
  }
  const logs = await invoiceAuditLog.default.findAll({
...
function registerInvoiceHandlers() {
      ...
      async (_event, invoiceItems, invoice2, postedBy) => {
        ...
        return await database.database.transaction(async (t) => {
          ...
          for (const item of invoiceItems) {
            ...
            if (!item.quantity) {
              throw new Error(
                `Quantity missing for ${product$1.title}`
              );
            }
            ...
            await product.default.decrement("stock", {
              by: item.quantity,
              where: { id: product$1.id },
              transaction: t
            });
            const computedAmount = item.quantity * item.unitPrice;
            const computedProfit = item.quantity * (item.unitPrice - product$1.buyPrice);
          ...
          await createInvoiceAuditLog({
            invoiceId: customerInvoice.id,
            action: "created",
            details: { saleType: invoice2?.saleType, amount: totalAmount },
            performedBy: invoice2?.postedBy ?? postedBy ?? "unknown",
            transaction: t
          });
      ...
      await database.database.transaction(async (t) => {
        ...
        await Promise.all(
          invoice$1.products.map(async (product$1) => {
            ...
            const stockBefore = product$1.stock;
            const stockAfter = stockBefore + invoiceItem2.quantity;
            await product.default.increment("stock", {
              by: invoiceItem2.quantity,
              where: { id: product$1.id },
              transaction: t
            });
            await productAuditLog.default.create(
        ...
        await createInvoiceAuditLog({
          ...
          action: "deleted",
          details: {
            saleType: invoice$1.saleType,
            amount: invoice$1.amount
          },
          performedBy: invoice$1.postedBy ?? "unknown",
          transaction: t
        });
    ...
    index.withAppReady(
      async (_event, { productId, invoiceId, invoiceItemId }) => {
        await database.database.transaction(async (t) => {
          ...
          const stockBefore = product$1.stock;
          const stockAfter = stockBefore + invoiceItem$1.quantity;
          await product.default.increment("stock", {
            by: invoiceItem$1.quantity,
            where: { id: productId },
            transaction: t
          });
          await productAuditLog.default.create(
          ...
          await createInvoiceAuditLog({
            invoiceId,
            action: "item_deleted",
            details: { productId, quantity: invoiceItem$1.quantity },
            performedBy: invoice$1.postedBy ?? "unknown",
            transaction: t
          });
    ...
    index.withAppReady(
      async (_event, currentInvoice, currentInvoiceItem) => {
        await database.database.transaction(async (t) => {
          ...
          if (!invoice$1) throw new Error("Invoice not found");
          const product$1 = await product.default.findByPk(
          ...
          if (existingItem) {
            ...
            const computedAmount = currentInvoiceItem.quantity * currentInvoiceItem.unitPrice;
            const computedProfit = currentInvoiceItem.quantity * (currentInvoiceItem.unitPrice - product$1.buyPrice);
            await invoiceItem.default.create(
              {
                invoiceId: invoice$1.id,
                productId: product$1.id,
          ...
          const oldInvoiceAmount = invoice$1.amount;
          await invoice$1.update(
  ...
  electron.ipcMain.handle(
    "invoice:getAuditLog",
    index.withAppReady(async (_e, invoiceId) => {
