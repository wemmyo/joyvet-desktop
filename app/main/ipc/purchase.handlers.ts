...
export function registerPurchaseHandlers(): void {
  ipcMain.handle(
    ...
    withAppReady(async (_event, input: unknown = {}) => {
      ...
      return toPaginatedResult(
        rows.map((purchase: any) => (purchase.toJSON ? purchase.toJSON() : purchase)),
        count,
        page,
        pageSize
      );
  ...
  ipcMain.handle(
    'purchase:update',
    withAppReady(async (_event, id: number, purchaseItems: any[], meta: any) => {
      const schema = z.object({
        values: z.array(z.any()),
        meta: z.object({
          invoiceNumber: z.string().min(1),
      ...
      await database.transaction(async (t: any) => {
        ...
        for (const each of purchaseItems) {
          ...
          if (each.newSellPrice !== undefined && each.newSellPrice !== (prod as any).sellPrice) {
            priceChanges.push({
              field: 'sellPrice',
              before: (prod as any).sellPrice,
              after: each.newSellPrice,
